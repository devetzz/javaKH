...
/* 
 * 3단계 변수명을 클래스(PhoneInfo)를 설계하여 관리
 * 이름, 전화번호, 나이, 성별
 * 클래스 배열로 n명을 입력받아서 출력하는 프로그램 작성
...
//					PhoneInfo phoneInfo = new PhoneInfo(name, phoneNum, age, gender);
					pi[i] = new PhoneInfo(name, phoneNum, age, gender);
				}
				break;
			case 2:
				...
				for(int i = 0; i < COUNT; i++) {
//					PhoneInfo p = pi[i];
					System.out.printf("%d번째 전화번호 : %-4s %10s %5d %4c\n", i+1, pi[i].getName(), pi[i].getPhoneNum(), pi[i].getAge(), pi[i].getGender());
				}
				break;
			case 3:
